...
# demand ~ UNIFORM
set.seed(1785)
blabou <- sim_ar_bis(n = NN+4)
past_demand_sequence <- blabou[1:NN]
demand_sequence <- blabou[1 + (1:NN)]
next_demand_sequence <- blabou[2 + (1:NN)]
...
for(iteration in 1:100){
  ...
  next_to_compute_reward <- data.frame(
    'past_demand' = initial_state[,'demand'],
  ...
  # fit model again :)
  network2 %>% fit(x = cbind(initial_state[,c('past_demand', 'storage')], decision_sequence)[-NN,], 
                   y = new_reward[-NN], 
                   epochs = 500, 
...
if(FALSE){
  fn <- 'model_2_policy_nostorage.RDS'
  if(file.exists(fn) == FALSE){
    saveRDS(object = bingaloo, file = fn)
  }
  fn <- 'model_2_data_generated_nostorage.RDS'
  if(file.exists(fn) == FALSE){
